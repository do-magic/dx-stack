...
describe('sync generator (end-to-end)', () => {
  ...
  it('generates a Dockerfile and maintains next.config.js for a Next.js app', async () => {
    ...
    const dockerfile = tree.read('apps/demo/Dockerfile', 'utf-8');
    expect(dockerfile).toContain('FROM source AS dev');
    expect(dockerfile).toContain('FROM base AS runner');

    const nextConfig = tree.read('apps/demo/next.config.js', 'utf-8');
  ...
  it('leaves an unrecognized-framework app with a hand-written Dockerfile alone', async () => {
    ...
    expect(tree.exists('apps/demo/next.config.js')).toBe(false);
