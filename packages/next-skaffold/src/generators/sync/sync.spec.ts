...
describe('sync generator', () => {
  ...
  describe('Dockerfile generation', () => {
    ...
    it('places the marker comment after the syntax directive, never before it', async () => {
      ...
      expect(lines[1]).toContain('Generated by @dxs/next-skaffold:sync');
    });
  ...
  describe('next.config.js maintenance', () => {
    it('creates a minimal next.config.js when the app has none', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
    ...
    it('adds both properties to an existing config, preserving everything else', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
      tree.write(
...
`,
    ...
    it('overwrites an incorrect value but leaves unrelated properties untouched', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
      tree.write(
        'apps/demo/next.config.js',
...
`,
    ...
    it('regenerates valid syntax when updating an existing last property while inserting a new one right after it', async () => {
...
`,
    ...
    it('leaves an already-correct config byte-for-byte unchanged', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
...
`;
    ...
    it('does not duplicate an existing path import', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
      tree.write(
        'apps/demo/next.config.js',
...
`,
    ...
    it('handles an inline module.exports object literal', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
      tree.write(
        'apps/demo/next.config.js',
...
`,
    ...
    it('handles an ES module "export default" config', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
      tree.write(
        'apps/demo/next.config.js',
...
`,
    ...
    it('rejects a next.config.js whose exported shape is not recognized', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo');
      tree.write(
        'apps/demo/next.config.js',
    ...
    it('does not touch next.config.js for a non-Next.js app with a hand-written Dockerfile', async () => {
      mockApps([{ name: 'demo', root: 'apps/demo' }]);
      addApp(tree, 'demo', {
        packageJson: { dependencies: { express: '^4.0.0' } },
      });
      ...
      tree.write('apps/demo/next.config.js', 'module.exports = {};\n');

      await syncGenerator(tree);

      expect(tree.read('apps/demo/next.config.js', 'utf-8')).toBe(
        'module.exports = {};\n',
      );
    });
  });
